    ...
    >
      ...
      <div class="wrapper text-white" v-if="!loading">
        <div v-if="data" class="flex gap-5 justify-between" id="hero-container">
          <div>
            <h1 class="font-bold text-3xl mb-5">{{ data.series.title }}</h1>
            <p>{{ data.series.overview }}</p>
            <p class="mt-5 mb-2 text-lg">Release: {{ data.series.release_date }}</p>
            <div class="flex gap-1 flex-wrap">
              <v-chip v-for="genre in data.series.genres" :key="genre">{{ genre.name }}</v-chip>
            </div>
          </div>
          <div class="flex flex-col gap-3" id="poster-container">
            <div class="w-[200px] h-[300px] bg-wexo rounded-md">
              <img
                class="rounded-sm"
                :src="`https://image.tmdb.org/t/p/w200${data.series.poster_path}`"
                :alt="data.title + ' series poster'"
              />
            </div>
            <v-btn color="slate" class="w-[200px] mt-3" @click="addToWatchlist(data.series.id)">Add to watchlist</v-btn>
          </div>
        </div>
      </div>
    </v-parallax>
...
import { authenticate, addSeriesToWatchlist, getSpecificSeriesData } from '@/services/tmdb.service.js';
...
const addToWatchlist = async (movieId) => {
  if (await authenticate()) {
    await addSeriesToWatchlist(movieId)
...
onMounted(async () => {
  data.value = await getSpecificSeriesData(route.params.id);
  loading.value = false;
});
</script>
